...
基于 Nuxt 4 的 SSG 网页工具箱，用于承载图片压缩、智能图片对比改名、批量缩放等浏览器端小工具。
...
路由：`/tools/image-resizer`

- 输入自定义目标像素（宽 × 高，1–16384），批量调整图片尺寸。
- 「保持原始图片比例」默认勾选：等比缩放到目标框内（如 2000×1000 在 1000×500 框内输出 1000×500）；取消后强制输出精确目标宽高。
- 支持 WebP、JPEG、PNG、AVIF 输入；输出格式可选保持原格式或转换为 WebP/JPEG/PNG，编码在本地 WASM Worker 中完成。
- 处理设置通过 `app/utils/useLocalStorage.ts` 保存到当前浏览器，刷新后仍有效。
- 文件只在当前浏览器中读取，不会上传；导出 ZIP 保留原目录结构。
