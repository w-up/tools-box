      ...
      <section class="migration-panel" aria-labelledby="merge-title">
        ...
        <div v-if="merges.length > 0" class="migration-merge-list">
          <div v-for="(merge, index) in merges" :key="index" class="migration-merge-row">
            <label>待移除图片<select v-model="merge.sourceNames" multiple size="4"><option v-for="image in projectImages" :key="image.id" :value="image.name" :disabled="isSourceInOtherMerge(image.name, index) || image.name === merge.targetName">{{ image.relativePath }}</option></select></label>
            <span class="migration-merge-arrow">→</span>
            <label>保留图片<select v-model="merge.targetName"><option v-for="image in projectImages" :key="image.id" :value="image.name">{{ image.relativePath }}</option></select></label>
